...
<div data-screen-label="Sign in" style={css(`--bg:#141312;--surface:#1d1c1b;--surface2:#242322;--border:rgba(255,255,255,0.09);--text:#ECEBE9;--muted:#9a9993;--faint:#6e6d6a;--accent:#F5330A;font-family:'Geist',-apple-system,BlinkMacSystemFont,sans-serif;background:var(--bg);color:var(--text);min-height:100vh;display:grid;grid-template-columns:1.05fr 1fr`)} className="si-grid">
  ...
  <div className="si-left" style={css(`position:relative;overflow:hidden;border-right:1px solid var(--border);background:linear-gradient(160deg,#1b1a18 0%,#141312 60%);padding:clamp(32px,3.4vw,52px);display:flex;flex-direction:column;justify-content:space-between;min-height:100vh`)}>
    ...
    <a href="/" style={css(`position:relative;display:flex;align-items:center;gap:10px;color:var(--text)`)}>
  ...
  <div className="si-right" style={css(`display:flex;align-items:center;justify-content:center;padding:clamp(28px,4vw,56px);min-height:100vh`)}>
    <div style={css(`width:100%;max-width:400px`)}>
      ...
      <div style={css(`display:flex;flex-direction:column;gap:11px;margin-bottom:22px`)}>
        <button type="button" onClick={v.onGoogle} className="btn btn--secondary btn--lg btn--block">
      ...
      <form onSubmit={v.onSubmit} style={css(`display:flex;flex-direction:column;gap:16px`)}>
        <label style={css(`display:block`)}>
          ...
          <input type="email" required placeholder="you@studio.com" style={css(`width:100%;background:var(--surface);border:1px solid var(--border);color:var(--text);font-size:15px;font-family:inherit;padding:13px 14px;border-radius:12px`)} />
        </label>
        <label style={css(`display:block`)}>
          <span style={css(`display:flex;justify-content:space-between;align-items:baseline;margin-bottom:8px`)}><span style={css(`font-size:13px;font-weight:600;color:var(--muted)`)}>Password</span><a href="#" style={css(`font-size:12.5px;font-weight:500`)}>Forgot?</a></span>
          <input type="password" required placeholder="••••••••" style={css(`width:100%;background:var(--surface);border:1px solid var(--border);color:var(--text);font-size:15px;font-family:inherit;padding:13px 14px;border-radius:12px`)} />
        </label>
      ...
      <p style={css(`margin:24px 0 0;text-align:center;font-size:14.5px;color:var(--muted)`)}>New here? <a href="/onboarding" style={css(`font-weight:600`)}>Create an account</a></p>
    </div>
  </div>
