...
/* Small pieces shared by the signup, login, forgot-password and reset screens,
...
/* An error tied to its input by id, announced to screen readers. Renders
   nothing at all when the field is fine, so there's no empty live region
   flickering under every input. */
export function FieldError({ id, message }) {
  if (!message) return null;
  return (
    <div className="hint err" id={id} role="alert">
      {message}
    </div>
  );
}

/* The neutral frame every auth screen sits in. */
